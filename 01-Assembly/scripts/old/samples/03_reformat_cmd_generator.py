...
############################################################
# Functions
def genReformatCmd(reformat_path, sfiles, r, baselogfile, indir, outdir, cmd_list):
    for f in sfiles:
        fint = os.path.join(indir, f);

        fout1 = f.replace(".fastp.decon.fastq.gz", "_R1_.fastp.decon.fastq.gz");
        fout2 = f.replace(".fastp.decon.fastq.gz", "_R2_.fastp.decon.fastq.gz");

        fout1 = os.path.join(outdir, fout1);
        fout2 = os.path.join(outdir, fout2);
...
parser.add_argument("-p", dest="path", help="The path to reformat.sh. Default: reformat.sh", default="reformat.sh");
...
dedup_dir = os.path.join(base_outdir, "03-Dedup");
...
# Generating the commands in the job file.
    for s in spec:
        ...
        for r in runtype:
            ...
            if not os.path.isdir(run_reformat_indir):
                continue;

            #seqfiles = mfiles.getFiles(s_mod, r, run_string, reformat_dir, unmerged_flag=False);
            seqfiles = [ f for f in os.listdir(run_reformat_indir) if ".fastq.gz" in f ];
            
            if seqfiles:
                reformat_cmds = genReformatCmd(args.path, seqfiles, r, base_logfile, run_reformat_indir, run_dedup_outdir, reformat_cmds);
