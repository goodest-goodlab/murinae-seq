...
############################################################
# Functions
def genMergeCmd(merge_path, sfiles, r, baselogfile, step, prev_step):
    cmd_list = [];
    cmd_num = 0;
    for f in sfiles:
        if r in [0,1]:
        ...
        # Single end runs
            ...
            merge_cmd = merge_path + " -Xmx15g t=1 in1=" + f[0] + " in2=" + f[1] + " verystrict=t rem k=40 extend2=40 ecct outu1=" + unmerged1 + " outu2=" + unmerged2 + " out=" + mergefile;
            cmd_num += 1;
...
# Generating the commands in the job file.
    for s in spec:
        ...
        for r in runtype:
            ...
            if seqfiles:
                merge_cmds = genMergeCmd(args.path, seqfiles, r, spec_dir, base_logfile, prev_step);
            ...
            if s_mod in ["Rattus-exulans", "Rattus-hoffmani"]:
                ...
                seqfiles = mfiles.getFiles(s_mod, r, run_string, prev_step_dir);
                if seqfiles:
                    merge_cmds_2 = genMergeCmd(args.path, seqfiles, r, spec_dir, base_logfile, prev_step);
                    merge_cmds += merge_cmds_2;
