...
parser.add_argument("-p", dest="path", help="The path to referee. Default: /mnt/beegfs/gt156213e/referee-dev/referee/referee.py", default="/mnt/beegfs/gt156213e/referee-dev/referee/referee.py");
...
# Generating the commands in the job file.
    for s in specs:
        ...
        spec_pileup = os.path.join(pileup_dir, s_mod, s_mod + ".mpileup");
        if ".." in spec_pileup:
            spec_pileup = spec_pileup.replace("..", ".");
        spec_assembly = os.path.join(assembly_dir, s_mod, s_mod + ".fa");
        if ".." in spec_assembly:
            spec_assembly = spec_assembly.replace("..", ".");

        assert os.path.isfile(spec_pileup), "\nCannot find pileup file: " + spec_pileup;
        assert os.path.isfile(spec_assembly), "\nCannot find assembly file: " + spec_assembly;

        spec_out_dir = os.path.join(referee_dir, s_mod);
        if not os.path.isdir(spec_out_dir):
            os.system("mkdir " + spec_out_dir);
        spec_prefix = s_mod + "-referee";
        logfile = os.path.join(logdir, s_mod + "-referee.log");

        #pileup_cmd = args.path + " mpileup -d 999999999 -f " + spec_assembly + " -Q 0 -s -B -o " + spec_out + " " + spec_bam + " &> " + logfile;

        referee_cmd = "python " + args.path + " -ref " + spec_assembly + " -gl " + spec_pileup + " --pileup --correct --fastq --fasta -p 8 -d " + spec_out_dir + " -prefix " + spec_prefix + " --overwrite &> " + logfile;

        mcore.PWS(referee_cmd, jobfile);
