...
# Generate the indices for the base-corrected, iupac genomes.
############################################################
...
varcall_dir = os.path.join(base_outdir, "10-Varcall");
...
with open(output_file, "w") as jobfile:
    ...
    mcore.PWS(mcore.spacedOut("# Varcall directory:", pad) + varcall_dir, jobfile);
...
# Generating the commands in the job file.
    for s in specs:
        ...
        spec_varcall_dir = os.path.join(varcall_dir, s_mod)
        assert os.path.isdir(spec_varcall_dir), "\nVarcall directory not found: " + spec_varcall_dir;
        spec_iupac_fa = os.path.join(spec_varcall_dir, s_mod + "-iupac-consensus.fa");
        assert os.path.isfile(spec_iupac_fa), "\nIUPAC file not found: " + spec_iupac_fa;

        picard_cmd = "picard CreateSequenceDictionary R=" + spec_iupac_fa + " O=" + spec_iupac_fa.replace(".fa", ".dict") + " &> " + os.path.join(logdir, s_mod + "-picard-dict.log");
        faidx_cmd = "samtools faidx " + spec_iupac_fa + " &> " + os.path.join(logdir, s_mod + "-faidx.log");
        index_cmd = "bwa index " + spec_iupac_fa + " &> " + os.path.join(logdir, s_mod + "-bwa-index.log");
