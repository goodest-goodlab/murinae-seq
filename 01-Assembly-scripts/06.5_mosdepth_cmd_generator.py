...
parser = argparse.ArgumentParser(description="Generates commands for generating mosdepth files.");
...
parser.add_argument("-p", dest="path", help="The path to mosdepth. Default: mosdepth", default="mosdepth");
...
# Generating the commands in the job file.
    for s in assembled_specs:
        ...
        if args.ref:
            ...
            logfile = os.path.join(logdir, s_mod + "-" + args.ref + "-mosdepth.log");

            mosdepth_cmd = args.path + " -b " + tile_file + " -x " + out_prefix + " " + spec_bam + " &> " + logfile;
        else:
            ...
            logfile = os.path.join(logdir, s_mod + "-mosdepth.log");

            mosdepth_cmd = args.path + " -x " + out_prefix + " " + spec_bam + " &> " + logfile;

        mcore.PWS(mosdepth_cmd, jobfile);
