...
parser.add_argument("-ref", dest="ref", help="The reference genome to use. Either 'mouse', 'mouse-tiles', or 'rat' or left blank to map back to exome assembly.", default=False);
...
if args.ref:
    if args.ref not in ['mouse', 'mouse-tiles', 'rat']:
...
read_dir = os.path.join(base_outdir, "04-Cat-fastq");
...
# Generating the commands in the job file.
    for s in assembled_specs:
        ...
        spec_read_dir = os.path.join(read_dir, s_mod);

        if args.ref == "mouse":
            spec_assembly = "/mnt/beegfs/gt156213e/ref-genomes/mm10/mm10.fa";
            spec_out_dir = os.path.join(bam_dir, s_mod + "-mouse");
        elif args.ref == "mouse-tiles":
            spec_assembly = "/mnt/beegfs/gt156213e/murinae-seq/Targets/tile-seqs/tiles-mm10-seqs.fa";
            spec_out_dir = os.path.join(bam_dir, s_mod + "-mouse-tiles");
        elif args.ref == "rat":
            spec_assembyl = "/mnt/beegfs/gt156213e/ref-genomes/rnor6/rn6.fa";
            spec_out_dir = os.path.join(bam_dir, s_mod + "-rat");
        else:
            spec_assembly = os.path.join(assembly_dir, s_mod, s_mod + ".fa");
            spec_out_dir = os.path.join(bam_dir, s_mod);
        ...
        assert os.path.isfile(spec_assembly), "\nAssembly file not found: " + spec_assembly;

        se, pe1, pe2, pem = False, False, False, False;

        pi_cmd = args.path + " -ref " + spec_assembly;

        for reads in os.listdir(spec_read_dir):
            if not reads.endswith(".fq.gz"):
                continue;

            full_reads = os.path.join(spec_read_dir, reads);

            if reads.endswith("-single.fq.gz"):
                pi_cmd += " -se " + full_reads;
                se = full_reads;
            if reads.endswith("-paired-1.fq.gz"):
                pi_cmd += " -pe1 " + full_reads;
                pe1 = full_reads;
            if reads.endswith("-paired-2.fq.gz"):
                pi_cmd += " -pe2 " + full_reads;
                pe2 = full_reads;
            if reads.endswith("-merged.fq.gz"):
                pi_cmd += " -pem " + full_reads;
                pem = full_reads;

        if (pe1 and not pe2) or (not pe1 and pe2):
            sys.exit( " * ERROR 3: Only one paired end read file found for the following species: " + s);

        pi_cmd += " -picard 'picard -Xmx10g' -tmp tmp-pi-out --maponly --keepall -p 1 -resume " + spec_out_dir + "/";
