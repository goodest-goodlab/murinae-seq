...
jobs_dir = os.path.join(cwd, "jobs", "06.5-Post-map-jobs");
# Job files
...
step_dir = os.path.abspath("../01-Assembly-data/06-Map/");
...
with open(output_file, "w") as jobfile:
    ...
    for s in spec:
        ...
        # run_status = [];
        # for r in range(len(spec_ids[s])):
        #     if r in runtype:
        #         run_status.append(spec_ids[s][r]);
        
        # #print(run_status);

        # if all(r == "NA" for r in run_status):
        #     continue;
        ...
        if args.ref == "-mouse":
            ...
            ref = os.path.join(assembly_dir, s_mod, s_mod + ".fa");
