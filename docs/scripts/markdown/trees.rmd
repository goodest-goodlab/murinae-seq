...
#tips_to_drop = c(missing_samples, tribe_prob_full, "Paruromys_dominator_JAE4870", "Lophiomys_imhausi_UM5152", "Mus_musculus-castaneus_10460X7", "Mus_musculus-musculus_10460X14")
...
# for(i in 1:nrow(labeled_timetree_info)){
#   if(!labeled_timetree_info[i,]$clade %in% labeled_tribetree_info$clade){
#     print(labeled_timetree_info[i,]$clade)
#   }
# }



speciation_tree = getMeanBranchLengthTree(best)
tree_to_df_list = treeToDF(speciation_tree$phy)
...
############
# START HERE TOMORROW
# GET SPECIATION RATES (EDGE.LABEL) ON TO SAME TREE AS DELTA
############
############
...
delta_labeled_tree = rodent_tree
delta_labeled_tree$node.label = paste(delta_labeled_tree$node.label, "/", tree_info$delta, sep="")

delta_tree = ggtree(speciation_tree$phy, size=0.8, ladderize=F) +
  #scale_color_continuous(name="Delta", high=h, low=l) +
  xlim(0, 25) +
  geom_tiplab(color="#333333", fontface='italic', size=2)
  #theme(legend.position=c(0.05,0.9)) +
