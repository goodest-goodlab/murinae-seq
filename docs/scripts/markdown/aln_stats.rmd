...
num_seqs = rbind(pre_seqs_150, post_seqs_150, pre_seqs_175, post_seqs_175, pre_seqs_mcl, post_seqs_mcl)

num_seqs$label = factor(num_seqs$label, levels=c("Pre-filter (150)", "Post-filter (150)", "Pre-filter (175)", "Post-filter (175)", "Pre-filter (McL F0)", "Post-filter (McL F0)"))
...
aln_seqs = ggplot(num_seqs, aes(x=label, y=num.seqs, fill=label)) +
  ...
  xlab("") +
  scale_fill_manual(labels=c("Pre-filter (150)", "Post-filter (150)", "Pre-filter (175)", "Post-filter (175)", "Pre-filter (McL F0)", "Post-filter (McL F0)"), values=filter_col) +
  scale_x_discrete(labels=c("Pre-filter (150)", "Post-filter (150)", "Pre-filter (175)", "Post-filter (175)", "Pre-filter (McL F0)", "Post-filter (McL F0)")) +
...
num_seqs = rbind(pre_seqs_150, post_seqs_150, pre_seqs_175, post_seqs_175, pre_seqs_mcl, post_seqs_mcl, gbe_seqs)
num_seqs = subset(num_seqs, aln.len <= 1000)

num_seqs$label = factor(num_seqs$label, levels=c("Pre-filter (150)", "Post-filter (150)", "Pre-filter (175)", "Post-filter (175)", "Pre-filter (McL F0)", "Post-filter (McL F0)", "GBE"))
...
aln_len = ggplot(num_seqs, aes(x=label, y=aln.len, fill=label)) +
  ...
  xlab("") +
  scale_fill_manual(labels=c("Pre-filter (150)", "Post-filter (150)", "Pre-filter (175)", "Post-filter (175)", "Pre-filter (McL F0)", "Post-filter (McL F0)", "GBE"), values=filter_col) +
  scale_x_discrete(labels=c("Pre-filter (150)", "Post-filter (150)", "Pre-filter (175)", "Post-filter (175)", "Pre-filter (McL F0)", "Post-filter (McL F0)", "GBE")) +
  #scale_y_continuous(expand=c(0,0)) +
  bartheme() +
  theme(legend.position="none",
        axis.text.x=element_text(angle=45, hjust=1))
...
```{r fig6, fig.align="center", warning=FALSE, out.width="100%"}
...
num_seqs = rbind(pre_seqs_150, post_seqs_150, pre_seqs_175, post_seqs_175, pre_seqs_mcl, post_seqs_mcl, gbe_seqs, mus_lens)
num_seqs = subset(num_seqs, nongap.len <= 1000)

num_seqs$label = factor(num_seqs$label, levels=c("Pre-filter (150)", "Post-filter (150)", "Pre-filter (175)", "Post-filter (175)", "Pre-filter (McL F0)", "Post-filter (McL F0)", "GBE", "Mus CDS"))
...
nongap_len = ggplot(num_seqs, aes(x=label, y=nongap.len, fill=label)) +
  ...
  xlab("") +
  scale_fill_manual(labels=c("Pre-filter (150)", "Post-filter (150)", "Pre-filter (175)", "Post-filter (175)", "Pre-filter (McL F0)", "Post-filter (McL F0)", "GBE", "Mus CDS"), values=filter_col) +
  scale_x_discrete(labels=c("Pre-filter (150)", "Post-filter (150)", "Pre-filter (175)", "Post-filter (175)", "Pre-filter (McL F0)", "Post-filter (McL F0)", "GBE", "Mus CDS")) +
  #scale_y_continuous(expand=c(0,0)) +
  bartheme() +
  theme(legend.position="none",
        axis.text.x=element_text(angle=45, hjust=1))
...
num_sites = rbind(pre_sites_150, post_sites_150, pre_sites_175, post_sites_175, pre_sites_mcl, post_sites_mcl)
num_sites = subset(num_sites, perc.sites < 1000)
num_sites$label = factor(num_sites$label, levels=c("Pre-filter (150)", "Post-filter (150)", "Pre-filter (175)", "Post-filter (175)", "Pre-filter (McL F0)", "Post-filter (McL F0)"))
...
```{r read-filter, out.width="100%", fig.align = "center", warning=FALSE}
...
Alignment length (codons)
```{r fig12, out.width="100%", fig.align = "center", warning=FALSE}
#gbe_seqs = select(gbe_data, align, pre.codon.aln.length)
gbe_data$label = "GBE"
names(gbe_data)[3] = "post.codon.aln.length"
gbe_lens = select(gbe_data, align, label, post.codon.aln.length)
...
Ungapped alignment length (codons)
```{r fig13, out.width="100%", fig.align = "center", warning=FALSE}
#gbe_seqs = select(gbe_data, align, pre.codon.aln.length)
gbe_data$label = "GBE"
names(gbe_data)[4] = "post.avg.nongap.length"
gbe_lens = select(gbe_data, align, label, post.avg.nongap.length)
...
Alignment length (codons)
```{r fig14, out.width="100%", fig.align = "center", warning=FALSE}
#gbe_seqs = select(gbe_data, align, pre.codon.aln.length)
gbe_data$label = "GBE"
names(gbe_data)[3] = "post.codon.aln.length"
gbe_lens = select(gbe_data, align, label, post.codon.aln.length)
...
Ungapped alignment length (codons)
```{r fig15, out.width="100%", fig.align = "center", warning=FALSE}
#gbe_seqs = select(gbe_data, align, pre.codon.aln.length)
gbe_data$label = "GBE"
names(gbe_data)[4] = "post.avg.nongap.length"
gbe_lens = select(gbe_data, align, label, post.avg.nongap.length)
