...
filter_data = read.csv("../../data/selected-transcripts-target-check.txt", header=T, sep="\t", comment.char='#')
...
ylabels = as.character(seq(from=-10000, to=10000, by=1000))
...
dualp = ggplot(binned_data, aes(x = bin, y = ifelse(filter == "Post-filter",-1, 1)*count, fill = filter)) + 
  ...
  scale_y_continuous(name = "# Transcripts", limits=c(-10000,10000), breaks = 1000*(-10:10), labels=ylabels) +
