...
2. Set a maximum number of iterations to perform and a maximum number of species to remove.
...
infile = here("docs", "data", "trees", "paring", paste("min-spec-paring-", tree_type, sep=""), paste("min-spec-paring-", tree_type, ".log", sep=""))
...
# Visualization of iterations using ASTRAL tree and the minimize species removed selection criterion
...
# Loop over every pruning iteration in the input data
  ...
  iter_tree_info$supp = as.numeric(iter_tree_info$label)
  ...
  b_tree = ggtree(iter_tree, size=0.8, ladderize=F, aes(color=iter_tree_info$low.b)) +
    scale_color_discrete(name = "Prune?") +
    xlim(0, xmax) +
...
```{r prune-tree, out.width="75%", fig.align = "center", warning=FALSE, fig.height=8}
